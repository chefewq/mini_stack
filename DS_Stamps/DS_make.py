...
# ---------------------------------------------------------------#
# ****************************************************************************************
def SAR_data_read(SLC_path_file, data_format, nofLines, nofPixels, Patch_First_Line, Patch_First_Pixel, Patch_nofLines,
                  Patch_nofPixels):
    R.acquire()
    print
    SLC_path_file
...
def freadbk(path_file, line_start=1, Pixels_start=1, nofLines1=None, nofPixels1=None):
    ...
    print
    'Band Type=', gdal.GetDataTypeName(cint_srd.DataType)

    if cint_srd.GetOverviewCount() > 0:
        print
        'Band has ', cint_srd.GetOverviewCount(), ' overviews.'
...
def process_control(ifg_para_file, process_control_name, rmstep_flag):
    ifg_process_control = int(
        get_parameter(process_control_name, ifg_para_file, 2, 'Start_process_control', 'End_process_control'))
...
def Optimization_wrap_phase(para_insar, Observation_data_array, Observation_coherence_array):
    ...
    return -1 * np.dot(Observation_coherence_array.ravel(),
                       np.cos(np.dot(A_matrix, para_insar) - Observation_data_array))
        ...
        if os.path.isfile(res_file_name):
            if int(master_time) == int(Master_date_time):
                ...
                cint_srd_data = np.conj(SAR_data_read(SLC_path_file, data_format, \
                                                      Total_nofLines, Total_nofPixels, 1, 1, Total_nofLines,
                                                      Total_nofPixels))
...
def main():
    ...
    if len(filename_list) >= 0:
        ...
        ##################################################################
            ...
            file_master_mini_stack = open(mini_stack_para, 'r')
            for line in file_master_mini_stack:
                print(line)
                master_i_temp=line.split(' ')[0].strip()
                print(master_i_temp)
                master_j_temp=line.split(' ')[1].strip()
                print(master_j_temp)
                filename_list.append(master_i_temp.strip())
                filename_list.append(master_j_temp.strip())
