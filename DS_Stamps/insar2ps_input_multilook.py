...
for filename in open('Interfergrom_combination.list'):
...
for insar_file_name in Interfergrom_combination_info:
    ...
    if file_time.strip()!=master_date.strip():
        ...
        print(dirname,"\n",filename)
        swath_num=filename.split('iw')[1][0].strip()
        print 'swath_num=',swath_num
        inner_original_fold=''
        #shutil.copy()
        print 'original_fold=',original_fold
        print 'target_fold=',target_fold
        ...
        inner_original_fold=''
        original_source=original_fold.strip()+'/'+inner_original_fold+'/'+'slave_rsmp_mul.res' 
        target_source  =target_fold+'/'+'slave.res'
        shutil.copy(original_source,target_source)

        original_source_res=original_fold+'/'+inner_original_fold+'/'+'slave_rsmp_mul.res' 
        ...
        original_source=original_fold+'/'+inner_original_fold+'/'+'slave_rsmp_mul.raw' 
        target_source  =target_fold+'/'+slave_rsmp_raw
        print 'target_fold=',target_fold
        ...
        original_source=original_fold+'/'+inner_original_fold+'/'+'merged_h2ph_srd_mul.raw' 
        ...
        burst_index_num=original_slave_data.split('.')[0].split('_')[-1].strip()
        original_source=original_fold+'/'+inner_original_fold+'/'+'burst'+burst_index_num+'/'+original_slave_data.strip()
        ...
        original_source_res=original_fold+'/'+inner_original_fold+'/'+master_date+'_iw_'+swath_num+'_mul.res'
...
if Insar_master_check==0:    
    ...
    original_source     =original_fold+'/'+inner_original_fold+'/'+master_date+'_iw_'+swath_num+'_mul.raw'
...
for insar_file_name in Interfergrom_combination_info:
    ...
    if file_time.strip()!=master_date.strip():
        ...
        inner_original_fold=''
        original_source=original_fold.strip()+'/'+inner_original_fold+'/'+'mergedIfgBursts_srd_mul.raw' 
...
python_instruction='mt_prep '+Amplitude_dispersion+' '+'1 9 '+' '+' 50 50'
cmd_profile_copy=Linux_cmd(python_instruction)
print 'cmd_profile_copy=',cmd_profile_copy 
