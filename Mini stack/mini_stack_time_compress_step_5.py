...
def PCA_insar(input_fun_cmd):
        ...
        ####################################################################################
            ...
            for dataFilename in cint_minrefdem_file:
                ...
                Coherence_Data=SAR_data_read(SLC_path_file,data_format,\
                               Total_nofLines,Total_nofPixels,Patch_First_Line+1,\
            ...
            Single_SLC_Data_Stack=np.zeros((len(cint_day_file)-1,int(Patch_nofLines),int(Patch_nofPixels)),dtype=np.complex64)
            print 'cint_day_file=',cint_day_file
            for file_cint  in cint_day_file:
                ...
                SLC_path_file  =SBAS_work_dir+'/'+temp_cint_file_name+'/'+'cint.minrefdem.raw'
              
               
                data_format='complex_real4'
                cint_data_init=SAR_data_read(SLC_path_file,data_format,\
                               Total_nofLines,Total_nofPixels,\
                               Patch_First_Line+1,Patch_First_Pixel+1,Patch_nofLines,Patch_nofPixels)
    ...
    #######################################################################################
            Resample_SLC_Data_Stack=np.zeros((len(Resample_day_file),int(Patch_nofLines),int(Patch_nofPixels)),dtype=np.complex64)
            file_count=0
            
            print 'Resample_day_file=',Resample_day_file
            for file_cint  in Resample_day_file:

                if file_cint.strip()==Master_time.strip():
                    SLC_path_file  =target_run_path+'/'+file_cint.strip()+'_crop.slc.cr4'                     
                else: 
                    SLC_path_file  =target_run_path+'/'+file_cint+'/'+'slave_res.slc'                     
                    
                cint_data_init=np.abs(SAR_data_read(SLC_path_file,data_format,Total_nofLines,\
                               Total_nofPixels,Patch_First_Line+1,Patch_First_Pixel+1,\
                               Patch_nofLines,Patch_nofPixels)) 

                if file_cint.strip()==Master_time.strip():
                    
                    cint_data_init=cint_data_init*exp(1j*np.zeros((int(Patch_nofLines),int(Patch_nofPixels)),dtype=np.complex64) )                   
                else: 
                    SLC_path_file  =target_run_path+'/'+file_cint+'/'+'cint.minrefdem.raw'     
                    cint_data_init=cint_data_init*exp(-1j*np.angle(SAR_data_read(SLC_path_file,data_format,Total_nofLines,\
                               Total_nofPixels,Patch_First_Line+1,Patch_First_Pixel+1,\
                               Patch_nofLines,Patch_nofPixels)))
               
                
                Resample_SLC_Data_Stack[file_count,:,:]=cint_data_init.copy()
            ...
            Resample_SLC_KS =Resample_SLC_Data_Stack.copy()  
    ######################################################################################
                        ...
                        if (temp_da > amplitude_dispersion_param_value) and (temp_line_ind > Limited_square_First_Line) \
                            ...
                            if sum_filter > sum_filter_wins:                            
                                ...
                                if np.min(np.real(eig_val)) > 1.0: # EMI. To avoid small negative eigenvalue.
                                    ...
                                    min_eig_vec=eig_vec[:,np.argmin(abs_eig_val)]
                                    min_eig_vec=min_eig_vec*conj(min_eig_vec[Relative_MASTER_DATE_INDEX])
                                   
                                    min_eig_vec=min_eig_vec/np.linalg.norm(min_eig_vec+0.0000000000001,ord=1)
                                   
                                    Resample_SLC_KS[Relative_MASTER_DATE_INDEX,Relative_line_ind,\
                                                    Relative_pixel_ind]=\
                                    np.abs(Resample_SLC_Data_Stack[Relative_MASTER_DATE_INDEX,\
                                           Relative_line_ind,Relative_pixel_ind])\
                                    *np.exp(1j*np.angle(dot(conj(min_eig_vec).reshape(1,-1),\
                                    Resample_SLC_Data_Stack[:,Relative_line_ind,Relative_pixel_ind])[0] ))
                                   
                                    red_eig_vec=np.angle(eig_vec[:,np.argmin(abs_eig_val)])
                                 
                                    Optimization_pca=red_eig_vec[Relative_MASTER_DATE_INDEX]-red_eig_vec
                                    Single_SLC_KS[:,Relative_line_ind,Relative_pixel_ind] = \
                                    np.exp(1j*Optimization_pca[Coherence_Ind])*np.abs(Single_SLC_Data_Stack[:,Relative_line_ind,Relative_pixel_ind])
                                    ...
                                    if (max_eig_val/np.sum(abs_eig_val)) > PCA_Energy_thres:
                                        ...
                                        max_eig_vec=max_eig_vec/np.linalg.norm(max_eig_vec+0.0000000000001,ord=1)
                                     
                                        Resample_SLC_KS[Relative_MASTER_DATE_INDEX,Relative_line_ind,Relative_pixel_ind]=\
                                    np.abs(Resample_SLC_Data_Stack[Relative_MASTER_DATE_INDEX,Relative_line_ind,Relative_pixel_ind])\
                                    *np.exp(1j*np.angle(dot(conj(max_eig_vec).reshape(1,-1),\
                                    Resample_SLC_Data_Stack[:,Relative_line_ind,Relative_pixel_ind])[0]))
                                ...
                                if int(string_line_ij[0])%50000==0:
            ...
            file_count=0 
            ###################################################
            print "wuwenhao  TEST NO RESULT"
            ###################################################
            print 'cint_day_file=',cint_day_file
            print int(Relative_master_mini_stack.strip())
            # time.sleep(1000000000000)
            for file_cint  in cint_day_file:
                print 'file_cint=',file_cint
                print 'file_cint=',int(file_cint)
                if int(file_cint) > int(Relative_master_mini_stack.strip()): 
                    temp_cint_file_name= Relative_master_mini_stack.strip()+'_'+file_cint.strip()
                if int(file_cint) < int(Relative_master_mini_stack.strip()):
                    temp_cint_file_name= file_cint.strip()+'_'+Relative_master_mini_stack.strip()
                ...
                Output_file    =SBAS_work_dir+'/'+temp_cint_file_name+'/'+'cint.ministack_'+patch_Name.strip()+'.raw'
            ...
            for file_cint  in Resample_day_file:
                ...
                if file_count==Relative_MASTER_DATE_INDEX:
                    Output_fold    =work_dir+'/'+'mini_stack_compress'+'/'+file_cint
                    if not os.path.exists(Output_fold):
                        os.makedirs(Output_fold)
                   
                    Output_file=Output_fold+'/'+'resample_ministack_'+patch_Name.strip()+'.raw'
                    print 'Output_file=',Output_file 
                    print 'Relative_master_mini_stack=',Relative_master_mini_stack
                    ...
                    outformat= 'complex_real4'
                    print 'outformat=',outformat
                    SAR_write(Output_file,Resample_SLC_KS[file_count,:,:],outformat)
...
##################################################################
        ...
        if os.path.exists(patch_list_name):
            ...
            fcntl.flock(f.fileno(),fcntl.LOCK_UN)
            f.close()
            #for patch_file in reversed(patch_list_content):
