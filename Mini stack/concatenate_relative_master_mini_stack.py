...
def main():
   ...
   ##############################################################
    ...
    Relative_master_work_dir=work_dir+'/Relative_master'
    print 'Relative_master_work_dir=',Relative_master_work_dir

    for cint_master_slave in open('day.1.in'):
        print 'cint_master_slave_name=',cint_master_slave.split(' ')
        #master_date=cint_master_slave.split(' ')[0].strip() 
        #slave_date=cint_master_slave.split(' ')[1].strip() 
        #print 'master_date=',master_date
        #print 'slave_date=',slave_date
        
        cint_master_slave=cint_master_slave.strip()
        #mini_stack_master_slave_list:
        #cint_master_slave=SBAS_work_dir+'/'+cint_master_slave.strip()
        #for cint_master_slave_name in open(cint_master_slave):
        
        #print 'cint_master_slave_name=',len(cint_master_slave_name.split('_')) 
        #print "len(cint_master_slave.split(' '))=",len(cint_master_slave.split('_'))
        if len(cint_master_slave)>0:
            #print 'cint_master_slave.strip()=' ,cint_master_slave.strip()                  
            cint_day_file.append(cint_master_slave.strip())
    print 'cint_day_file=',cint_day_file 
    
    for file_cint  in cint_day_file:

        Output_file_path    =os.getcwd()+'/'+'Relative_master'+'/'+file_cint.strip()
        print 'Output_file_path=',Output_file_path
        if not os.path.exists(Output_file_path):
            continue
        #master_name, slave_name  = file_cint.split('_',2)

        #print  'master_name=',master_name 
        #data_init_path = '/home/wuwenhao/data_disk/three_gorage/three_gorage_staMPS_2/INSAR_20180211/SMALL_BASELINES'
        SLC_path_file  =SBAS_work_dir+'/'+file_cint.strip()+'/'+'cint.minrefdem.raw'     
        Path_MFF_HDR   =SBAS_work_dir+'/'+file_cint.strip()+'/'+'cint.minrefdem'+'.hdr'
        Link_DATA      =SBAS_work_dir+'/'+file_cint.strip()+'/'+'cint.minrefdem'+'.x00'
        ...
        #print  'Single_SLC_Data=',Single_SLC_Data[109,300:310]
        #print  'Single_SLC_Data=',Single_SLC_Data[309,300:310]  
        #Single_SLC_Data.astype(np.complex64)
        #Output_file    =os.getcwd()+'/'+file_cint.strip()+'/'+'cint.mini.stack.raw'
        ...
        for patch_Name in open('patch.list'):
            ...
            Read_file      =Relative_master_work_dir+'/'+file_cint.strip()+'/'+'cint.relative_master'+'_'+patch_Name.strip()+'.raw'            
            print 'Read_file=',Read_file

            if os.path.exists(Read_file)==True:
 
                Path_MFF_HDR   =Relative_master_work_dir+'/'+file_cint.strip()+'/'+'cint.relative_master'+'_'+patch_Name.strip()+'.hdr'
                Link_DATA      =Relative_master_work_dir+'/'+file_cint.strip()+'/'+'cint.relative_master'+'_'+patch_Name.strip()+'.x00'
                ...
                #print 'slc_cut=',slc_cut[600:610,1400:1410]
                ...
                print 'can not find th file'
                #usage()
                #sys.exit(1)
                #value=None
        Single_SLC_Data=Single_SLC_Data.astype(np.complex64)
        Output_file    =Relative_master_work_dir+'/'+file_cint.strip()+'/'+'cint.relative_master.raw'
         
        #if os.path.exists(Output_file)==True:
        #    os.remove(Output_file)
        print 'Output_file=',Output_file
        if Valid_Read_file==1:
        ...
        ' -P'+str(Crop_Image_width)+' '+Output_file+' '+'  > '+Relative_master_work_dir+'/interferogram_mix.ras'
                cmd_profile_copy=Linux_cmd(python_instruction)
                print 'cmd_profile_copy=',cmd_profile_copy

                jpg_name=os.getcwd()+'/'+'Relative_master'+'/'+file_cint.strip()+'_crop.jpg'
                python_instruction='convert '+Relative_master_work_dir+'/interferogram_mix.ras '+' '+jpg_name
                cmd_profile_copy=Linux_cmd(python_instruction)
                print 'cmd_profile_copy=',cmd_profile_copy  
