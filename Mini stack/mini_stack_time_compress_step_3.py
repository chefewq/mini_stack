...
def SAR_data_read(SLC_path_file,data_format,nofLines,nofPixels,Patch_First_Line,Patch_First_Pixel,Patch_nofLines,Patch_nofPixels):
    ...
    #   os.remove(Path_MFF_HDR)
        ...
        print 'Path_MFF_HDR is already here'
        #if os.path.exists(Path_MFF_HDR)==False:
    else:
        print 'Path_MFF_HDR=',Path_MFF_HDR
        print 'Path_MFF_HDR is touching'  
    ...
    if (os.path.isfile(Link_DATA)):
        ...
        print 'Link_DATA is already here'
    else:        
        os.symlink(SLC_path_file ,Link_DATA)
        print 'Link_DATA is touching'
        ...
        if os.path.isfile(res_file_name):
            if int(master_time)==int(Master_date_time):
                ...
                slave_data=SAR_data_read(SLC_path_file,data_format,\
                        Total_nofLines,Total_nofPixels,1,1,Total_nofLines,Total_nofPixels)
                cint_srd_data=slave_data*np.exp(-1.0j*np.angle(master_data)) 
...
#******************************************************************************        
#####################################################################################
def CInter_combine_operation(input_fun_cmd):
    work_dir=input_fun_cmd[0]
    original_work_dir=input_fun_cmd[-1]
    target_run_path=input_fun_cmd[0]
    #print 'input_fun_cmd=',input_fun_cmd
   ...
   #Doris_operation(SBAS_master_slave_list)
#################################################################################
#################################################################################
###############################################################################################
#Master_date='20170330'
#######################################################
...
for ks_stack_num in large_mini_stack:
    ...
    Reference_master_date=master_date
        ...
        for i_index in mini_stack_file_name_list:     
            ...
            for j_index in mini_stack_file_name_list:
                if int(j_index) > int(i_index):
                    master_time=i_index
                    slave_time=j_index 
                    temp_combine_file_name=master_time.strip()+' '+slave_time.strip()
                    mini_stack_combine_list.append(temp_combine_file_name)
                    temp_combine_file_name=master_time.strip()+'_'+slave_time.strip()
                    mini_stack_combine_file.append(temp_combine_file_name)           

       
    print  'mini_stack_combine_list=', mini_stack_combine_list
    if os.path.exists(Master_Date_fold):
       
        output_small_baseline_file= Master_Date_fold+'/small_baselines.list'          
        python_file=open(output_small_baseline_file,'w')
        for i_index_name in mini_stack_combine_list:
            python_file.writelines(i_index_name +'\n')
        python_file.close()
        ...
        target_run_path= os.path.abspath(Master_Date_fold)
        print 'target_run_path=',target_run_path

        filename_list=mini_stack_file_name_list
        input_cmd=[]
        input_cmd.append(target_run_path)
        input_cmd.append(filename_list)
        input_cmd.append(mini_stack_combine_file)
        Relative_MASTER_DATE_INDEX=int(len(filename_list)/2)
        Reference_master_date=filename_list[Relative_MASTER_DATE_INDEX]
        #Relative_MASTER_DATE_INDEX=filename_list.index(Reference_master_date) 
        print 'Relative_MASTER_DATE_INDEX=',Relative_MASTER_DATE_INDEX
        print 'Reference_master_date=',Reference_master_date 
        input_cmd.append(Relative_MASTER_DATE_INDEX)
        
        if int(filename_list[Relative_MASTER_DATE_INDEX])==int(Reference_master_date):
           
            input_cmd.append(work_dir)
            print 'input_cmd=',input_cmd
           
            CInter_combine_operation(input_cmd)
